/* 2) Implement the following operations in Open Interval Tree :
    a. Check overlap.
*/
...
struct IntervalTreeNode *searchTreeNode(struct IntervalTreeNode *node, struct IntervalTreeNode *root, int low, int high)
{
    if (root == NULL)
    {
        printf("EMPTY\n");
    ...
    if ((node->low <= low && low <= node->high) || (node->low <= high && high <= node->high) || (node->low >= low && high >= node->high))
    {
        printf("Yes\n");
        return node;
    }
    ...
    if (node->left != NULL && node->left->max >= low)
        return searchTreeNode(node->left, root, low, high);
    else
        return searchTreeNode(node->right, root, low, high);
    return NULL;
}
...
int main()
{
    ...
    while (option != 3)
    {
        ...
        case 2:
            ...
            scanf("%d", &tmp);
            scanf("%d", &tmp2);
            searchTreeNode(tree, tree, tmp, tmp2);
            break;
        case 4:
            inorderTravers(tree);
            break;
